...
1. Создать файл `.env` с переменной `PROD_HOST`. При разработке использовалось значение `9119`, можно использовать любое другое, однако нужно будет внести изменение в команду в п.4. Шаг опциональный - в случае отсутствия файла или переменной `PROD_HOST` будет использовано значение `9119` по умолчанию.
2. В терминале выполнить команду `git clone https://creds@github.com/AlexGribPython/BGPBTestTask.git` (`creds` будут высланы по электронной почте вместе с заданием) для клонирования репозитория с проектом.
3. В терминале выполнить команду `docker build: docker build -t image_name .` для создания `Docker`-образа.
4. В терминале выполнить команду `docker run -p 9119:9119 image_name` для запуска контейнера.
